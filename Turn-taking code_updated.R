# Turn-taking predicts vocabulary acquisition 

data <- read.csv("data/kuhl_dataset.csv")
names(data)
...
gghglm <- ggplot(data, aes(x = Participant)) +
  ...
  geom_smooth(aes(y = Prediction, color = "Vocabulary predicted"),
              method = "loess", se = FALSE) +
  labs(
    x = "Participants ordered by vocabulary size",
    y = "Vocabulary at 18 months",
    color = "Values"
  ) +
  scale_color_manual(values = c("Vocabulary observed" = "black",
                                "Vocabulary predicted" = "red")) +
...
ggsave(
  filename = "Turn_taking_Feurstein.pdf",
  plot = gghglm,
  path = "C:/Users/lvfeu/Desktop/PhD/University of Zurich/Paper 1",
  width = 16,
  height = 9,
